...
export function FormField({
  ...
  return (
    <label className='flex flex-col gap-1 rounded-2xl border border-[var(--line)] bg-white/70 px-4 py-3 shadow-sm shadow-black/5'>
      <div className='flex items-center justify-between text-sm font-semibold text-[var(--ink-strong)]'>
        <span>{label}</span>
        {suffix ? (
          <span className='text-xs font-medium text-[var(--ink-muted)]'>{suffix}</span>
        ) : null}
      </div>
      <div className='flex items-center gap-2 rounded-xl bg-[var(--panel)] px-3 py-2 ring-1 ring-[var(--line)]'>
        {prefix ? (
          <span className='text-sm font-semibold text-[var(--ink-muted)]'>{prefix}</span>
        ) : null}
        <input
